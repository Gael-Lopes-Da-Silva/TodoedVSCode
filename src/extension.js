...
function listKeywords() {
	...
	Object.keys(keywords).forEach(keyword => {
		...
		while ((match = regex.exec(text))) {
			...
			const contentAfterSymbol = match[3] ? ` ${symbol}${match[3].trim()}` : (match[10] ? ` ${symbol}${match[10].trim()}` : '');
...
function updateDecorations() {
	...
	Object.keys(keywords).forEach(keyword => {
		...
		let match;
		while ((match = regex.exec(text))) {
			const startPos = activeTextEditor.document.positionAt(match.index + match[0].lastIndexOf(keyword));
			const endPos = activeTextEditor.document.positionAt((match.index + match[0].lastIndexOf(keyword)) + keyword.length);

			const keywordIndex = match[0].indexOf(match[1] || match[2]) || 0;
			const line = activeTextEditor.document.positionAt(match.index + keywordIndex).line + 1;
